...
class Host extends Cf_REST_Controller
{
    ...
    function count_get()
    {
        try
        {
            $result = cfpr_host_count($this->username, $this->param('colour'), $this->param_includes(), $this->param_excludes());
    ...
    function compliance_get($arg = NULL)
    {
        try
        {
            ...
            {
                case 'timeseries':
                    $this->respond_ok(cfpr_host_compliance_timeseries($this->username, $this->param_includes(), $this->param_excludes()));

                case 'timeseries_shifts':
                    $this->respond_ok(cfpr_host_compliance_timeseries_shifts($this->username, $this->param_includes(), $this->param_excludes()));
