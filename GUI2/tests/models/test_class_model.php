...
class test_class_model extends CodeIgniterUnitTestCase {
    ...
    public function tearDown() {
     ...
     public function test_getClassList(){
         $name = $this->_ci->class_model->getAllClasses($this->username, array( ".*"), array(''), 10,1);
         $this->assertTrue(is_array(json_decode($name)),"Must be a valid set of class");
         $this->dump($name); 
    }
    
    public function test_getClassListWithParam(){
         $name = $this->_ci->class_model->getAllClasses($this->username, $this->default_include, $this->default_exclude, 10,1);
         $this->assertTrue(is_array(json_decode($name)),"Must be a valid set of class");
         $this->dump($name); 
    }
    ...
    public function test_getAllTimeClassesWithParam(){
         $name = $this->_ci->class_model->getAllTimeClasses($this->username, $this->default_include, $this->default_exclude, 10,1);
         $this->assertTrue(is_array(json_decode($name)),"Must be a valid set of class");
         $this->dump($name); 
    }
    ...
    public function test_getAllSoftClassesWithParam(){
        $name = $this->_ci->class_model->getAllSoftClasses($this->username, $this->default_include, $this->default_exclude,10,1);
        $this->assertTrue(is_array(json_decode($name)),"Must be a valid set of class");
        $this->dump($name); 
    }
