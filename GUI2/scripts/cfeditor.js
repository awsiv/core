...
$(document).ready(function() {
    ...
    var tab_counter = 2;
    var code_editor_counter=1;//previously 0
    ...
    var $tabs = $('#tabs').tabs({
        ...
        select: function(event, ui) {
            ...
            if(editors[current_tab_id]) {
                // editors[current_tab_index].setOption("mode","CFEngine");
    ...
    .click(function() {
        ...
        addTab('new');
        editors[current_tab_id] = CodeMirror.fromTextArea(document.getElementById('code'+code_editor_counter), {
            onKeyEvent: function(i, e) {
                // Hook into ctrl-space
                if (e.keyCode == 32 && (e.ctrlKey || e.metaKey) && !e.altKey) {
                    e.stop();
                    return startComplete(current_tab_id);
                }
            },
            lineNumbers:false
        });
        code_editor_counter++;
        $(current_tab_id).find('.CodeMirror').height($(current_tab_id).height())
    });
    ...
    $('#tabs span.ui-icon-close').live('click', function() {
        ...
        if(tabtype=='codeEditor')//if the tab is of type editor and it should complete couple of things befor closing.
        {
            ...
            newcontents =editors[tab_id].getValue();
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data.status)
                        {
                            ...
                            {
                                var remtabtoclose=current_tab_index;
                                ...
                                $tabs.tabs('remove',remtabtoclose);
                            }
    ...
    .click(function() {
        ...
        var agent=jQuery.uaMatch(navigator.userAgent).browser;
        var newcontents =editors[current_tab_id].getValue();
        ...
        buttons: {
            'Save & Commit': function() {
                ...
                $.ajax({
                    ...
                    url: "cfeditor/save_contents",
                    data:({
                        'file':$('#tobesaved_name',this).val(),
                        'content':$('#tobesaved',this).val(),
                        'filestats':'old',
                        'agent':agent
                    }),
...
function shortcuts(){
    ...
    shortcut.add("Ctrl+n",function() {
	 ...
	 $('#checksyntax').trigger('click');
        });

      shortcut.add("Ctrl+q",function() {
         $("a[href='"+current_tab_id+"']").siblings('span.ui-icon-close').trigger('click');
        });
