(function($) {
    ...
    {     
        ...
        options: {
            ...
            defaultbehaviour: false, //d
            width: 700,
            height: 600,
            subscribe: '',
            autoopen : true, //d
            itemId: 'itemId',
            placeholderId: 'itemId_wrapper',
            sortable: false,
...
/* todo: we should discuss if we are going to limit class view, because sometimes user won't see class list at all         
         if (self.options.defaultContext != '') {
            self.setContext(self.options.defaultContext[0], self.options.defaultContext[1]);
         }
*/         
        ...
        _create:function(){
            ...
            self.addsearchbar();
            self.addalphapager();
            if (self.options.addAlphabetFilter == true)
            {
                self.addalphapager();
            }
        ...
        addsearchbar:function(){
            var self = this;
            if (self.options.addSearchBar == true) {
            ...
            //var el = self.dialogcontent.parent();
            var el = $('div#clfres')

                el.addClass('customdlg').removeClass('ui-widget-content');

                //self.titlebar = self.dialogcontent.siblings('div.ui-dialog-titlebar');

                self.titlebar = $('div#clfres .ui-dialog-titlebar');


                self.filter = $('<ul>');
                self.filter.attr('id', 'cfinderfilter').addClass('classfilters').appendTo(el);

                self.gethostbtn = $('<span>').attr('id', 'findmatchedhost').text('view hosts');
                self.gethostbtn.appendTo(el).hide();
                ...
                self.matchhostfinder.appendTo(el);
                ...
                el.delegate('#findmatchedhost','click',$.proxy(self.findmatchedhost,self));
        ...
        loadDataInContainer: function(data,append) {
            ...
            var previousListElement = $('#'+self.options.placeholderId).find('ul.classList');
            ...
            var sortableClass = this.options.sortableConnectionClass;
            ...
            if (self.options.sortable == false) {
                if (append) {
                    ul = '<ul class="classList '+ sortableClass + '">' + previousList + list.join('') + '</ul>';
                } else {
                    ul = '<ul class="classList '+ sortableClass + '">' + list.join('') + '</ul>';
                }
            }
            else {
             if (append) {                
                    ul=  '<ul id="' + itemId + '" class="itemlist '+ sortableClass + '">' +previousList + list.join('') + '</ul>';
                } else {
                    ul=  '<ul id="' + itemId + '" class="itemlist '+ sortableClass + '">' + list.join('') + '</ul>';
                } 
            }

            
            //document.getElementById(self.dialogId).innerHTML = ul;
            document.getElementById(self.options.placeholderId).innerHTML = ul;

            self.element.text(self.elementtext);
            self.revertTitle();
            
            // bind sortable after list creatred
            if (self.options.sortable == true)
            {
                bindSortable(self.options.sortableConnectionClass, self.options.sortableDestinations);
            }
        },
        ...
        addalphapager: function()
        {
            ...
            if (self.options.addAlphabetFilter == true) {     
                ...
                self.alphasearch.appendTo($('div#clfres'));
