(function($){
    ...
    {     
        ...
        addsearchbar:function(){
            if (this.options.addSearchBar == true) {
                var self =this;
        ...
        loadDataInContainer:function(data,append) {
            ...
            var previousListElement = document.getElementById(itemId);
            ...
            var ul = '';
            if (append) {                
                ul=  '<ul id="' + itemId + '" class="itemnlist '+ sortableClass + '">' +previousList + list.join('') + '</ul>';
            } else {
                ul=  '<ul id="' + itemId + '" class="itemlist '+ sortableClass + '">' + list.join('') + '</ul>';
            }   


            document.getElementById(this.options.placeholderId).innerHTML = ul;
            
            var el = self.dialogcontent.find('#'+itemId);
            el.delegate('a','click',$.proxy(self.classSelected,self));
            el.delegate('a.classadd','click',$.proxy(self.addclassfilter,self));
        ...
        dialogContainer: function() {
            ...
            //set id for the element with all content

            var existing = $("#" + this.options.placeholderId);
            
            if ( existing.length != "") {
                return $("#" + this.options.placeholderId).first();
            }
            else {
               this.options.placeholderId =  this.options.itemId + "_wrapper";
               //single shared element for modal dialogs
                var requestDialog = $('<div id="'+ this.options.placeholderId +'" style="display:block" class="result" title=""><ul id="'+ this.options.itemId  +'"></ul></div>').appendTo('body').
        ...
        destroy: function(){
            ...
            document.getElementById(this.options.placeholderId).innerHTML = '';
            var self = this;
            self.resetPagination();
            self.resetSelectedLetter();
            // remove this instance from $.ui.mywidget.instances
            var element = this.element,
            position = $.inArray(element, $.ui.classfinderbox.instances);
            // if this instance was found, splice it off
            if(position > -1){
                $.ui.classfinderbox.instances.splice(position, 1);
            }
