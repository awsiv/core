...
PHP_FUNCTION(cfpr_ldap_authenticate)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "sss|b", &host, &h_len, &dn, &d_len, &passwd, &p_len, &starttls)
        == FAILURE)
    {
...
*/
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "ssssssssll|b",
                              ...
                              &name, &n_len,
                              &scopes, &s_len,
                              &sec, &ss_len, &passwd, &p_len, &page, &linesperpage, &starttls) == FAILURE)
    {
        zend_throw_exception(cfmod_exception_args, LABEL_ERROR_ARGS, 0 TSRMLS_CC);
        RETURN_NULL();
    }
    ...
    if (Nova2PHP_LDAPGetSingleAttributeList
        (uri, user, basedn, filter, name, scopes, sec, passwd, starttls, page, linesperpage, buffer, 1000000,
         &errmsg) == 0)
    {
...
*/
{
    ...
    buffer[0] = '\0';
    ret =
        Nova2PHP_LDAPGetSeveralAttributes(uri, user, basedn, filter, names, scopes, sec, passwd, starttls, page,
                                          linesperpage, buffer, 1000000, &errstr);
...
PHP_FUNCTION(cfpr_hosts_with_software_in)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "sssssbs",
                              &userName, &user_len,
                              &hostkey, &hk_len,
                              &name, &n_len, &version, &v_len, &arch, &a_len, &regex, &classreg, &cr_len) == FAILURE)
    {
        zend_throw_exception(cfmod_exception_args, LABEL_ERROR_ARGS, 0 TSRMLS_CC);
        RETURN_NULL();
    }
...
PHP_FUNCTION(cfpr_hosts_with_patch_in)
{
    ...
    Nova2PHP_software_hosts(fhostkey, fname, fversion, farch, regex, cfr_patch_installed, filter, buffer,
                            sizeof(buffer));
    DeleteHubQuery(hqHostClassFilter, DeleteHostClassFilter);
...
PHP_FUNCTION(cfpr_hosts_with_compliance_summary)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "sssllllss",
                              &userName, &user_len,
                              &hostkey, &hk_len,
                              &version, &v_len, &t, &k, &nk, &r, &cmp, &cmp_len, &classreg, &cr_len) == FAILURE)
    {
        zend_throw_exception(cfmod_exception_args, LABEL_ERROR_ARGS, 0 TSRMLS_CC);
        RETURN_NULL();
    }
    ...
    Nova2PHP_compliance_hosts(fhostkey, fversion, (int) t, (int) k, (int) nk, (int) r, cmp, filter, buffer,
                              sizeof(buffer));
    DeleteHubQuery(hqHostClassFilter, DeleteHostClassFilter);
...
PHP_FUNCTION(cfpr_hosts_with_performance)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "sssbs",
                              &userName, &user_len,
                              &hostkey, &hk_len, &job, &j_len, &regex, &classreg, &cr_len) == FAILURE)
    {
        zend_throw_exception(cfmod_exception_args, LABEL_ERROR_ARGS, 0 TSRMLS_CC);
        RETURN_NULL();
    }
...
PHP_FUNCTION(cfpr_hosts_with_setuid)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "sssbs",
                              &userName, &user_len,
                              &hostkey, &hk_len, &file, &j_len, &regex, &classreg, &cr_len) == FAILURE)
    {
        zend_throw_exception(cfmod_exception_args, LABEL_ERROR_ARGS, 0 TSRMLS_CC);
        RETURN_NULL();
    }
...
PHP_FUNCTION(cfpr_hosts_with_filechanges)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "sssblss",
                              &userName, &user_len,
                              &hostkey, &hk_len,
                              &file, &j_len, &regex, &t, &cmp, &c_len, &classreg, &cr_len) == FAILURE)
    {
        zend_throw_exception(cfmod_exception_args, LABEL_ERROR_ARGS, 0 TSRMLS_CC);
        RETURN_NULL();
    }
