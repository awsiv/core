...
{  struct Item *purgeKeys = NULL, *ip1,*ip2, *lastseen=NULL;
  struct Item *purgePcNames = NULL, *purgeClassNames = NULL;
  mongo_cursor *cursor;
  bson query,field,hostQuery,op;
  bson_iterator it1;
  bson_buffer bb,*unset;
  bson_buffer *pullAll, *arr,*obj;
  char removeKey[CF_MAXVARSIZE]={0};
  time_t now;
  int i=0;
  mongo_connection conn;
  int removeEntry = false;
...
i = ListLen(lastseen) - 1;

for(ip1=lastseen; ip1 != NULL; ip1 = ip1->next, i >= 0, i--)
  {
  removeEntry = false;

  for(ip2=list; ip2 != NULL; ip2 = ip2->next)
     {          
     if(strcmp(ip1->name,ip2->name) == 0 && strcmp(ip1->classes,ip2->classes) != 0)
        {
        removeEntry = true;
        }
     else if(strcmp(ip1->name,ip2->name) != 0 && strcmp(ip1->classes,ip2->classes) == 0)
        {
        removeEntry = true;
        }
     }
  ...
  if(removeEntry)
     ...
     // use of unset can be dangerous if null values are present in mongodb
     // so we pull out all null values in the next step
     
     pullAll = bson_append_start_object(&bb, "$unset");

     snprintf(removeKey,sizeof(removeKey),"lastseen_hosts.%d",i);
     bson_append_int(pullAll, removeKey, 1);

     bson_append_finish_object(pullAll);
     bson_from_buffer(&field,&bb);
     
     mongo_update(&conn, MONGO_SCRATCH, bson_empty(&query), &field, MONGO_UPDATE_MULTI);
     bson_destroy(&field);
     
     MongoCheckForError(&conn,"Remove array entries in the lastseen cache",NULL,NULL);
     }
  }
...
bson_buffer_init(&bb);
pullAll = bson_append_start_object(&bb, "$pull");

bson_append_null(pullAll, "lastseen_hosts");
...
mongo_update(&conn, MONGO_SCRATCH, bson_empty(&query), &field, MONGO_UPDATE_MULTI);
bson_destroy(&field);
MongoCheckForError(&conn,"Remove null entries from the lastseen cache",NULL,NULL);
  
if (!CFDB_Close(&conn))
  {
    CfOut(cf_verbose,"", "!! Could not close connection to cache database");
    return false;
  }
