 ...
 */
{
    ...
    // no licenses present yet when bootstrapping
    if (!BOOTSTRAP && !Nova_CheckLicenseWin("NovaWin_GetProcessSnapshot"))
    {
        return false;
    }
...
static int EnableDebugPriv(void)
{
    ...
    if ((AdjustTokenPrivileges(hToken, FALSE, &tkp, 0, NULL, NULL) != ERROR_SUCCESS)
...
static void FormatCreationTime(FILETIME *timeCreate, char *timeCreateStr)
{
    ...
    {
        if (timeCreateLocal.wMonth == timeCurrent.wMonth && timeCreateLocal.wDay == timeCurrent.wDay)   // today, print time as hh:mm
        {
...
static void GetMemoryInfo(HANDLE procHandle, char *memSzStr, DWORDLONG totalPhysMemB)
{
    ...
    sprintf(memSzStr, "%4.1f %8lu %8lu", (memInfo.WorkingSetSize * 100.0) / totalPhysMemB, memInfo.PrivateUsage / 1024,
