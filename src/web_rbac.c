...
/*****************************************************************************/
/* Knowledge pruning is fundamentally different to the host data model       */
/*****************************************************************************/

int RBACPruneKnowledge(char *topic, char *context, char *username)

{
    char topic_qualified[CF_BUFSIZE], search[CF_BUFSIZE];
    char topic_short[CF_BUFSIZE], ctx[CF_BUFSIZE];
    int topic_id;
 
    if (context == NULL)
    {
        strncpy(topic_qualified, topic, CF_BUFSIZE);
    }
    else
    {
    ...
    /*
      Look for any promise that is a member of a forbidden bundle, or
      a bundle that is called by a forbidden bundle. The relevant
      ...
      to see whether a bundle makes a promise or not, even if we can find our way to the promise
      indirectly.
      
    */

    snprintf(search, CF_BUFSIZE, "%s|%s|%s", KM_PARTOF_CERT_F, NOVA_IMPACTS, KM_USES_CERT_B);
    topic_id = Nova_GetTopicIdForTopic(topic_qualified);
    
    // Find bundles that the current topic belongs to
    
    Item *nn = Nova_NearestNeighbours(topic_id, search);
    
    if (nn)
    {
        for (Item *ip = nn; ip != NULL; ip = ip->next)
        {
            if (ip->counter == topic_id)
            {
                continue;
            }

            // We only want to restrict access based on bundle membership

            if (strcmp(ip->classes, "bundles") == 0 && RBACBundleIsAllowed(ip->name, username))
            {
                DeleteItemList(nn);
                return true;
            }
...

{
    ...
    // Semantics: if include exists, exclude everything except that which is included
    ...
    if (rbac->bundleRxExclude && FullTextMatch(rbac->bundleRxExclude, qualified_bundle) == true)
    {
        DeleteHubQuery(hqRBAC, DeleteHubUserRBAC);
        return false;
    }
