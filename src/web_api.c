...
int Nova2PHP_software_report(char *name,char *value, char *arch,int regex,char *returnval,int bufsize)
...
hq = CFDB_QuerySoftware(&dbconn,bson_empty(&b),name,value,arch,false);
...
for (rp = hq->records; rp != NULL; rp=rp->next)
   ...
   snprintf(buffer,CF_BUFSIZE,"<tr><td>%s</td><td>%s</td><td>%s</td></tr>\n",hs->name,hs->version,Nova_LongArch(hs->arch));
