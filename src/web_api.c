...
# ifndef NDEBUG
    if (IsEnvMissionPortalTesting())
    {
        return Nova2PHP_promiselog_summary_test(hostkey, handle, state, from, to, hostClassFilter, page, returnval,
                                                bufsize);
    }
# endif
    ...
    for (rp = hq->records; rp != NULL; rp = rp->next)
    {
        ...
        if (strcmp(hb->nid, CF_NONOTE) == 0)
        {
            snprintf(buffer, sizeof(buffer), "[\"%s\",\"%s\",%ld,%.2lf,%.2lf,%.2lf,"
                     ...
                     hb->hrsago, hb->hrsavg, hb->hrsdev, hb->hh->keyhash, CFREPORT_BUNDLE, hb->bundle);
        }
        else
        {
            snprintf(buffer, sizeof(buffer), "[\"%s\",\"%s\",%ld,%.2lf,%.2lf,%.2lf,"
                     "[\"show\",\"\",\"\",\"\",\"%s\"]],",
                     hb->hh->hostname, hb->bundle, hb->t, hb->hrsago, hb->hrsavg, hb->hrsdev, hb->nid);
        }
        margin = headerLen + noticeLen + strlen(buffer);
        if (!JoinMargin(returnval, buffer, NULL, bufsize, margin))
        {
...
int Nova2PHP_filechanges_report(char *hostkey, char *file, int regex, time_t t, char *cmp,
                                HostClassFilter *hostClassFilter, PageInfo *page, int lookInArchive, char *returnval,
                                int bufsize)
{
    char buffer[CF_BUFSIZE];
    HubFileChanges *hC;
                              ...
                              int bufsize)
{
    ...
    hq = CFDB_QueryFileDiff(&dbconn, hostkey, file, diffs, regex, t, icmp, true, hostClassFilter, lookInArchive);
...
int Nova2PHP_performance_hosts(char *hostkey, char *job, int regex, HostClassFilter *hostClassFilter, char *returnval,
                               int bufsize)
{
    char buffer[CF_BUFSIZE];
    HubHost *hh;
